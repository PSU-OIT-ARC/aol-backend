...
def import_lakes():
    ...
    def import_lake(row):
        try:
            queryset = Lake.objects.filter(Q(reachcode=row.get('reachcode')) |
                                           Q(permanent_id=row.get('permanent_id')) |
                                           Q(gnis_id=row.get('gnis_name')))

            # Legacy data matches an existing lake: update that lake metadata
            if queryset.exists():
                print("Updating existing lake '{}'...".format(row.get('reachcode')))
                return Lake.objects.update_or_create(defaults=row,
                                                     pk=row.get('reachcode'))

            # Emit a warning about the failed match
            print("Lake failed to match on reachcode, permanent_id and gnis_name")
            print("Legacy data: {}".format(row))
            # Load the legacy data into the database for future
            # matching and/or processing.
            row['title'] = '{} (Legacy)'.format(row.get('title', ''))
            print("Processing lake '{}'...".format(row.get('reachcode')))
            return Lake.objects.create(**row)
